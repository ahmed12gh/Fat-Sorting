...
/// list of files is structure that is used the handle the display of files in the program view it should be create use `new()`  and `create_list()` tha last funtions return gtk::scrolledWindow struct that should be the child of the window then files that is being displayed is added it via "set_files(Vec<PathBuf>)" finally the view should be updated using `update_list()`
...
fn create_list(files: &Vec<PathBuf>, listoffiles: &mut ListOfFiles) -> gtk::ScrolledWindow {
    ...
    let drag_scroll_ctl = gtk::DropControllerMotion::builder().build();

    // enbale drag scroll at the top and bottem of the window
    let accel = Rc::new(Cell::new(0.0f64));
    drag_scroll_ctl.connect_motion(clone!( @strong scroll  , @strong accel=> move |_,_,y|{
...
fn get_pathbuf_by_filename(filename: &str, listoffiles: &ListOfFiles) -> Option<PathBuf> {
    // error parsing & in gtk should be replaced back to & in the actual file path
    let filename = filename.replace("&amp;", "&");
    // this funcition should not panic or return none as all files haven't changed
